% UFinchGUIv2b reads a NHDFlowline (shapefile), a StreamGageEvent
...
%
function varargout = UFinchGUIv2b(varargin)
% UFINCHGUIV2B MATLAB code for UFinchGUIv2b.fig
%      UFINCHGUIV2B, by itself, creates a new UFINCHGUIV2B or raises the existing
%      singleton*.
%
%      H = UFINCHGUIV2B returns the handle to a new UFINCHGUIV2B or the handle to
%      the existing singleton*.
%
%      UFINCHGUIV2B('CALLBACK',hObject,eventData,handles,...) calls the local
%      function named CALLBACK in UFINCHGUIV2B.M with the given input arguments.
%
%      UFINCHGUIV2B('Property','Value',...) creates a new UFINCHGUIV2B or raises the
%      existing singleton*.  Starting from the left, property value pairs are
%      applied to the GUI before UFinchGUIv2b_OpeningFcn gets called.  An
%      unrecognized property name or invalid value makes property application
%      stop.  All inputs are passed to UFinchGUIv2b_OpeningFcn via varargin.
%
...
% Edit the above text to modify the response to help UFinchGUIv2b

% Last Modified by GUIDE v2.5 26-Aug-2014 13:29:08

% Begin initialization code - DO NOT EDIT
gui_Singleton = 0;
gui_State = struct('gui_Name',       mfilename, ...
    'gui_Singleton',  gui_Singleton, ...
    'gui_OpeningFcn', @UFinchGUIv2b_OpeningFcn, ...
    'gui_OutputFcn',  @UFinchGUIv2b_OutputFcn, ...
    'gui_LayoutFcn',  [] , ...
    'gui_Callback',   []);
...
%
% --- Executes just before UFinchGUIv2b is made visible.
function UFinchGUIv2b_OpeningFcn(hObject, eventdata, handles, varargin)
% This function has no output args, see OutputFcn.
% hObject    handle to figure
% eventdata  reserved - to be defined in a future version of MATLAB
% handles    structure with handles and user data (see GUIDATA)
% varargin   command line arguments to UFinchGUIv2b (see VARARGIN)

% Choose default command line output for UFinchGUIv2b
...
% UIWAIT makes UFinchGUIv2b wait for user response (see UIRESUME)
% uiwait(handles.figure1);
% --- Outputs from this function are returned to the command line.
function varargout = UFinchGUIv2b_OutputFcn(hObject, eventdata, handles)
...
for t = max(ttNetwork)+1:nSim,        % time frame for solution
    ...
    if mod(t,96)==0
        % Moved annotation from 110 (in line with "Unit to Daily Values" to 220
        annotation('rectangle','Units','pixels','FaceColor',...
            [0.5 0.5 0.5],'Position',[765,255,183*t/nSim,15]); 
        drawnow;
    end
end
%
...
fprintf(1,'Stored "beg" in hUFinchGUI\n');
...
% Sort ComID
sComID = sort(ComID);
travTime15 = 120;   % Average travTime15 to reference 
travTime = round(11/3*travTime15); 
% Form timedate vector for UFINCH flows
vYrMoDaHrMn  = sYrMoDaHrMn:1/96:eYrMoDaHrMn;
% Compute year vector
dateNum        = floor(datenum(vYrMoDaHrMn(1:end-travTime)));
dQMatrix       = NaN(3658,1729);
tic
for i = 1:1729,
    fprintf(1,'Compute daily means for the %u ComID %s at %6.2f minute\n',...
        i,num2str(sComID(i)),toc/60);
    uQUfinch                  = eval(['C',num2str(sComID(i))]);
    dQMatrix(:,i) = [grpstats(uQUfinch(1+travTime:end),dateNum,'mean')];
end
%
% Populate cell array with ComIDs for variable names
varNames = cell(1729,1);
for i = 1:1729,
    varNames{i} = ['C',num2str(sComID(i))];
    rowNames{i} = datestr(vYrMoDaHrMn(i),'yyyy-mmm-dd');
end
% 
uniqDate = unique(dateNum);
% Populate cell array with dates for row names
rowNames = cell(3658,1);
for j = 1:3658
    rowNames{j} = datestr(uniqDate(j),'yyyy-mmm-dd');
end
%
dailyFlows01636500 = array2table(dQMatrix,...
...
% --- Executes on button press in simUnitFlowPlotPB.
function simUnitFlowPlotPB_Callback(hObject, eventdata, handles)
% hObject    handle to simUnitFlowPlotPB (see GCBO)
% eventdata  reserved - to be defined in a future version of MATLAB
% handles    structure with handles and user data (see GUIDATA)
%
hUFinchGUI       = getappdata(0,'hUFinchGUI');
...
[fname, pname]      = uigetfile({['..\',huc02,'\FlowData\',gageNumber,...
    '.uFlo*']},'Open Unit Flow File for Selected Streamgage');
% set(handles.file15flowST,'String',[pname,fname]);
fid = fopen([pname,fname],'rt');
Q      = cell2mat(textscan(fid,'%f %f %f %f %f %f','Delimiter',','));
...
for h = 1:length(trvlTimeLagSeq),
    rmseLagTT(h) = sqrt(nansum( (simFlowVec(ndxSim(1+trvlTimeLagSeq(h)):ndxSim(end)) - ...
...
clf();
subplot(2,2,[1,2]);
semilogy(simTimeVec(1:end-tDelay15+1),simFlowVec(tDelay15:end),'r-');
%
xlabel('Date');
ylabel('Flow, in cubic feet per second');
title(['Hydrograph of Unit Flows at ',staNum,' ',staName,' with optimum time delay of ',...
    num2str(tDelay15),' hours, and a velocity acceleration factor of ',num2str(proVelAdj)]);
datetick('x');
% Plot the time adjusted simulated flows
hold on
% Plot the measured flow
semilogy(meaTimeVec(ndxMea),meaFlowVec(ndxMea),'b-');
legend('Simulated','Meaured');
hold off
%
% Plot the relation between measured and simulated flows in log space
subplot(2,2,3);
loglog(meaFlowVec(ndxMea(1:end-tDelay15+1)),simFlowVec(tDelay15:end),'b.',...
    'MarkerSize',1);
...
title({['Relation Between Measured and Simulated Flows at ',staNum,' ',staName],...
...
[yyyy,mm,dd,hr,mn,~] = datevec(simTimeVec(1:end-tDelay15+1));
%
