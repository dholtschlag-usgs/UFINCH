function varargout = ModelErrorGUIv0_1(varargin)
% MODELERRORGUIV0_1 MATLAB code for ModelErrorGUIv0_1.fig
%      MODELERRORGUIV0_1, by itself, creates a new MODELERRORGUIV0_1 or raises the existing
%      singleton*.
%
%      H = MODELERRORGUIV0_1 returns the handle to a new MODELERRORGUIV0_1 or the handle to
%      the existing singleton*.
%
%      MODELERRORGUIV0_1('CALLBACK',hObject,eventData,handles,...) calls the local
%      function named CALLBACK in MODELERRORGUIV0_1.M with the given input arguments.
%
%      MODELERRORGUIV0_1('Property','Value',...) creates a new MODELERRORGUIV0_1 or raises the
%      existing singleton*.  Starting from the left, property value pairs are
%      applied to the GUI before ModelErrorGUIv0_1_OpeningFcn gets called.  An
%      unrecognized property name or invalid value makes property application
%      stop.  All inputs are passed to ModelErrorGUIv0_1_OpeningFcn via varargin.
%
...
% Edit the above text to modify the response to help ModelErrorGUIv0_1

% Last Modified by GUIDE v2.5 07-Oct-2014 11:33:19

% Begin initialization code - DO NOT EDIT
gui_Singleton = 1;
gui_State = struct('gui_Name',       mfilename, ...
    'gui_Singleton',  gui_Singleton, ...
    'gui_OpeningFcn', @ModelErrorGUIv0_1_OpeningFcn, ...
    'gui_OutputFcn',  @ModelErrorGUIv0_1_OutputFcn, ...
    'gui_LayoutFcn',  [] , ...
    'gui_Callback',   []);
...
% --- Executes just before ModelErrorGUIv0_1 is made visible.
function ModelErrorGUIv0_1_OpeningFcn(hObject, eventdata, handles, varargin)
% This function has no output args, see OutputFcn.
% hObject    handle to figure
% eventdata  reserved - to be defined in a future version of MATLAB
% handles    structure with handles and user data (see GUIDATA)
% varargin   command line arguments to ModelErrorGUIv0_1 (see VARARGIN)

% Choose default command line output for ModelErrorGUIv0_1
...
plotECDF_Callback(handles.plotSimulatedUnitFlowsPB, eventdata, handles)


% UIWAIT makes ModelErrorGUIv0_1 wait for user response (see UIRESUME)
% uiwait(handles.figure1);


% --- Outputs from this function are returned to the command line.
function varargout = ModelErrorGUIv0_1_OutputFcn(hObject, eventdata, handles)
...
elseif mdl.NumCoefficients == 4
    [p05, p50, p95] = prctile(tbl.dif1LogSimFlow,[5,50,95]);
...
quadParm = get(handles.quadTermCB,'Value');
dif1Parm = get(handles.dif1TermCB,'Value');
dif2Parm = get(handles.dif2TermCB,'Value');
...
if      linrParm==1 && quadParm==1 && dif1Parm==1 && dif2Parm==0
    fprintf(1,'A quadratic model with differential correction will be used as the error model\n');
    ...
    set(handles.dif2LogSimMagET,'String','NaN');
    ...
    set(handles.dif2LogSimStdET,'String','NaN');
    ...
    set(handles.dif2LogSimTstatET,'String','NaN');
    ...
    set(handles.dif2LogSimPvalET,'String','NaN');
    ...
    %
elseif  linrParm==1 && quadParm==1 && dif1Parm==0 && dif2Parm==0
...
elseif   linrParm==1 && quadParm==0 && dif1Parm==0 && dif2Parm==0
...
elseif linrParm==0 && quadParm==0 && dif1Parm==0 && dif2Parm==0
    ...
    % Change background color of push button to plot static estimates
    set(handles.plotSttCorUnitFlowsPB,'BackgroundColor',[0.831, 0.816, 0.784]);
elseif linrParm==1 && quadParm==1 && dif1Parm==1 && dif2Parm==1
    fprintf(1,'A constant model will be used to describe the error\n');
    % Develop a constant regression model
    mdl = fitlm(tbl,...
        'logSimErr ~ 1 + logSimFlow + logSimFlow^2 + dif1LogSimFlow + dif1LogSimFlow^2',...
        'RobustOpt',rb)
    disp(mdl);
    ypred   = predict(mdl);
    % Populate table of parameters
    ...
    set(handles.dif2LogSimMagET,'String',num2str(mdl.Coefficients.Estimate(5)));    
    ...
    set(handles.dif2LogSimStdET,'String',num2str(mdl.Coefficients.SE(5)));    
    ...
    set(handles.dif2LogSimTstatET,'String',num2str(mdl.Coefficients.tStat(5)));    
    ...
    set(handles.dif2LogSimPvalET,'String',num2str(mdl.Coefficients.pValue(5)));    
...
plotSttCorUnitFlowsPB_Callback(handles.plotSttCorUnitFlowsPB, eventdata, handles);
plotStaticErrorModelPB_Callback(handles.plotStaticErrorModelPB, eventdata, handles);
...
% Hints: get(hObject,'Value') returns position of slider
%        get(hObject,'Min') and get(hObject,'Max') to determine range of slider
% Get handle to ModelErrorGUIv0_1 memory space
hUFinchGUI = getappdata(0,'hUFinchGUI');
...
% --- Executes on button press in plotECDF.
function plotECDF_Callback(hObject, eventdata, handles)
% hObject    handle to plotECDF (see GCBO)
% eventdata  reserved - to be defined in a future version of MATLAB
% handles    structure with handles and user data (see GUIDATA)
...
if ~isempty(sttFlowVec)
    ...
    fprintf(1,'Relative Area between Quantiles of Statically Corrected and Measured flow: %f\n',...
        areaStt2Mea);
    legend('Measured','Simulated','Static Correction','Location','NorthWest');
else
...
[fName,pName] = uiputfile('C:\Home\Projects\UFINCH\Analysis\UFinch\HR02\FlowData\Simulated\',...
...
function dif2LogSimMagET_Callback(hObject, eventdata, handles)
% hObject    handle to dif2LogSimMagET (see GCBO)
% eventdata  reserved - to be defined in a future version of MATLAB
% handles    structure with handles and user data (see GUIDATA)

% Hints: get(hObject,'String') returns contents of dif2LogSimMagET as text
%        str2double(get(hObject,'String')) returns contents of dif2LogSimMagET as a double


% --- Executes during object creation, after setting all properties.
function dif2LogSimMagET_CreateFcn(hObject, eventdata, handles)
% hObject    handle to dif2LogSimMagET (see GCBO)
% eventdata  reserved - to be defined in a future version of MATLAB
% handles    empty - handles not created until after all CreateFcns called
...
% --- Executes on button press in dif2TermCB.
function dif2TermCB_Callback(hObject, eventdata, handles)
% hObject    handle to dif2TermCB (see GCBO)
% eventdata  reserved - to be defined in a future version of MATLAB
% handles    structure with handles and user data (see GUIDATA)

% Hint: get(hObject,'Value') returns toggle state of dif2TermCB



function dif2LogSimStdET_Callback(hObject, eventdata, handles)
% hObject    handle to dif2LogSimStdET (see GCBO)
% eventdata  reserved - to be defined in a future version of MATLAB
% handles    structure with handles and user data (see GUIDATA)

% Hints: get(hObject,'String') returns contents of dif2LogSimStdET as text
%        str2double(get(hObject,'String')) returns contents of dif2LogSimStdET as a double


% --- Executes during object creation, after setting all properties.
function dif2LogSimStdET_CreateFcn(hObject, eventdata, handles)
% hObject    handle to dif2LogSimStdET (see GCBO)
% eventdata  reserved - to be defined in a future version of MATLAB
% handles    empty - handles not created until after all CreateFcns called
...
function dif2LogSimPvalET_Callback(hObject, eventdata, handles)
% hObject    handle to dif2LogSimPvalET (see GCBO)
% eventdata  reserved - to be defined in a future version of MATLAB
% handles    structure with handles and user data (see GUIDATA)

% Hints: get(hObject,'String') returns contents of dif2LogSimPvalET as text
%        str2double(get(hObject,'String')) returns contents of dif2LogSimPvalET as a double


% --- Executes during object creation, after setting all properties.
function dif2LogSimPvalET_CreateFcn(hObject, eventdata, handles)
% hObject    handle to dif2LogSimPvalET (see GCBO)
% eventdata  reserved - to be defined in a future version of MATLAB
% handles    empty - handles not created until after all CreateFcns called
...
function dif2LogSimTstatET_Callback(hObject, eventdata, handles)
% hObject    handle to dif2LogSimTstatET (see GCBO)
% eventdata  reserved - to be defined in a future version of MATLAB
% handles    structure with handles and user data (see GUIDATA)

% Hints: get(hObject,'String') returns contents of dif2LogSimTstatET as text
%        str2double(get(hObject,'String')) returns contents of dif2LogSimTstatET as a double


% --- Executes during object creation, after setting all properties.
function dif2LogSimTstatET_CreateFcn(hObject, eventdata, handles)
% hObject    handle to dif2LogSimTstatET (see GCBO)
% eventdata  reserved - to be defined in a future version of MATLAB
% handles    empty - handles not created until after all CreateFcns called
